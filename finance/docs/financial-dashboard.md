...
- `inv-carteira.js` renderiza a view Carteira: cards resumo (valor total, custo, P&L, TIR, posições — `count = positions.length` para incluir RF/funds que sempre carregam `quantity=0`), três donuts de alocação lado a lado — **por classe**, **por instituição** e **por moeda** (R$ / US$ / Crypto, tudo convertido em BRL — Crypto é separada de R$ porque é classe de ativo distinta apesar do `portfolio.json` cotar cripto em BRL nativo; bucket por position: `asset_class === 'crypto'` → Crypto, senão `currency === 'USD'` → US$ usando `current_value_brl`, senão R$ usando `current_value_brl ?? current_value`. Computado em JS no `invCurrencyAllocationDataset` porque `summary.allocation_by_currency` do calculate.py só conhece BRL/USD e não isola cripto). Cada donut tem (a) legenda em branco via `generateLabels` — `fontColor: '#ffffff'` é forçado por-item porque o `labels.color` default é ignorado quando `generateLabels` retorna items custom no Chart.js v4 — e (b) datalabels externos via `chartjs-plugin-datalabels` com `anchor: 'end'`, `align: 'end'`, offset 6px mostrando valor compacto em BRL — `R$ Xk` milhares, `R$ X,Xm` milhões, `R$ X` < mil — ao redor de TODAS as fatias (sem threshold de % — fatias pequenas como cripto sub-5% precisam aparecer); `layout.padding` 28/32px reserva espaço externo para os labels não clipparem nas bordas; canvas em 300px de altura; o plugin é registrado por-chart via `plugins: [ChartDataLabels]` no construtor para não vazar nas barras das outras views. Grid `repeat(auto-fit,minmax(280px,1fr))` mantém os 3 lado a lado em desktop e empilha em telas estreitas. As antigas tabelas "Posições por instituição" e "Alocação por moeda" foram removidas — os donuts são suficientes. Restantes: indicadores de mercado, seletor de cut date (dropdown populado via `snapshots.json`). Indicadores de mercado: cada card lê `value` + `change_1d` (ratio fracionário, ex.: `-0.005` = `-0,50%`); valor é formatado por chave (`IBOVESPA` inteiro com separador de milhar, `SP500` 2 decimais, `USD_BRL` 4 decimais, `BTC_BRL` prefixo `R$` inteiro), change é colorido verde/vermelho.
  - **TIR por classe** (`invCarteiraIrrBreakdown`): linha de chips entre os cards resumo e a decomposição RV EUA, um chip por bucket de `summary.irr.per_class` (ordem fixa `rv_br`, `rv_eua`, `rf_balcao`, `fundos`, `crypto`; bucket sem TIR e sem terminal não renderiza). Cada chip mostra a TIR all-time do bucket (verde ≥ 0, vermelho < 0, cinza null) + valor terminal em BRL como linha principal, mais uma segunda linha `Atual: +X%` (menor, colorida verde/vermelho/cinza-null, classe `privacy-hide`) com a TIR do variant `current` de `summary.irr.current.per_class`. A segunda linha só renderiza quando `summary.irr.current` estiver presente — snapshots legados sem esse bloco renderizam exatamente como antes (sem linha "Atual"). Cada chip carrega **tooltip nativo** (`title` + `cursor:help`, textos em `_INV_IRR_CLASS_TOOLTIPS`) reescritos para descrever AMBAS as taxas por bucket: (a) a TIR all-time (XIRR sobre todos os fluxos já registrados, incluindo posições encerradas, com as três mudanças semânticas de D12); (b) a TIR current (XIRR sobre fluxos de posições abertas no cut, com `terminal_value` compartilhado). Detalhes de denominação/composição por bucket: equities/crypto all-time incluem posições encerradas; balcão all-time inclui produtos vencidos/resgatados; `rv_eua` converte fluxos USD→BRL ao câmbio médio de funding com terminal ao câmbio atual — difere da coluna TIR da Bolsa, que roda em USD nativo. A conversão dos fluxos de tickers encerrados usa a cadeia de fallback estável do FX engine (`last_nonzero_avg_rate`, decisão D11 em `docs/investimentos.md`) — sobrevive a uma repatriação total do saldo Avenue sem degenerar o chip para FX 1,0.
- `inv-bolsa.js` renderiza a view Bolsa (filtra `valuation_method === 'price'` — listed equities, ETFs, FIIs, BDRs, opções, direitos de subscrição):
  ...
  - **Tabela de posições** (colapsável, aberta por default): qty / preço / PM / valor / P&L / retorno % / TIR / Ret. USD / FX / BRL All-in / proventos / YoC LT / YoC TTM / 1ª compra + variações 1d/30d/90d/180d/365d/YTD (via `price_changes`) + indicador de fonte (dot verde api, cinza snapshot, âmbar snapshot >60d, vermelho missing). Colunas sortáveis — default `current_value` desc. YoC (Yield on Cost) é computado em `calculate.py` a partir de `total_dividends / cost_basis` (LT = lifetime) e `total_dividends_ttm / cost_basis` (TTM = trailing 12 months). Funciona sem preços ao vivo — depende apenas dos ledgers `orders.csv` e `proventos.csv`.
  - **Decomposição de retorno (Ret. USD / FX / BRL All-in)**: só para posições não-BRL (`return_usd`, `return_fx_brl`, `return_total_brl` do `portfolio.json`); posições BRL e sem preço renderizam `—`. O valor da coluna FX carrega **tooltip nativo** (`title` + `cursor:help`, mesmo padrão do dot de qualidade da TIR no Balcão) decompondo o resultado cambial: `FX médio da posição: R$ A → FX atual: R$ B — efeito sobre o custo USD investido (US$ C)`. FX médio é o implícito no custo (`cost_basis_brl / cost_basis`) — NÃO `funding_fx`, que é qty-ponderado e não fecha a identidade `return_fx_brl = cost_basis × (FX atual − FX médio)`; FX atual vem de `meta.fx_rate_usd_brl` do portfolio carregado (snapshot-consistente; snapshots legados sem o campo renderizam sem tooltip). `meta.fx_rate_usd_brl` é resolvido em `calculate.py` via `_spot_usd_brl_rate`: `weighted_avg_rate` → `last_nonzero_avg_rate` (D11) → constante 5.0 (sempre anunciada no stderr — nunca silenciosa); veja D13 em `docs/investimentos.md`. Taxas com 4 decimais (`invBolsaFxTooltip`).
...
- `inv-balcao.js` renderiza a view Balcão (cobre `valuation_method === 'balcao'` — RF tradicional + todos os fundos, incluindo FIRF DI). Consome o breakdown per-operação que `position_calculator.py` agrega do balcão ledger: `aplicado_total`, `juros_amort_total`, `resgates_total`, `impostos_total`, `net_flow` (signed). `pnl_absolute = current_value + net_flow` (identidade algébrica de current_value − aplicado + juros_amort + resgates − impostos); quando não há snapshot, `pnl_absolute = null` e a posição cai no bucket "Sem valoração". Metadados de RF (`issuer`, `indexer`, `rate`, `indexer_pct`, `application_date`, `maturity_date`) vêm de `assets.csv` via `calculate.py`. TIR per-asset é calculada via XIRR sobre os cash flows do balcão (`irr_calculator.compute_xirr`):
  ...
  - **Dot de qualidade da TIR**: células TIR das tabelas RF e Fundos prefixam um dot âmbar com tooltip nativo quando `irr_quality ∈ {seed_only, short_window, unverified}` (`invBalcaoIrrQualityDot` + `_INV_BALCAO_IRR_QUALITY_TOOLTIPS`); `complete` renderiza só o número. O dot renderiza mesmo com `irr: null` (TIR `—`) — o sinal importa mais quando a TIR nem pode ser calculada. O dot é só display; a ordenação numérica por TIR não é afetada.
  - **Âncora temporal da TIR (Balcão)**: a TIR por posição ancora o terminal no `price_date` do snapshot, não no cut date — lê-se "XIRR até a última marcação" e não muda com o cut date enquanto os dados não mudam. `snapshot_age_days` segue mostrando a idade da marcação (decisão D9 em `docs/investimentos.md`).
