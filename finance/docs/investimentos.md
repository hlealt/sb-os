...
| D13 | Spot USD/BRL fallback chain for portfolio valuation | `build_portfolio` resolves the spot rate (`_spot_usd_brl_rate`, `calculate.py`) as live `weighted_avg_rate` → `last_nonzero_avg_rate` (D11) → constant 5.0, with the constant always announced on stderr (`[irr_calculator] spot FX: …`) — never silent. Same family as D11: pre-fix, `weighted_avg_rate or 5.0` let the constant silently value USD positions (`current_value_brl`, `meta.fx_rate_usd_brl`) after a full repatriation zeroed the live average. Masked when the `USD_BRL` market indicator overrides the value; real exposure on `--skip-prices` runs or indicator-fetch failures. |
...
**Spot USD/BRL fallback (portfolio valuation, D13).** The spot rate that values USD positions (`current_value_brl`, `total_dividends_brl`, `meta.fx_rate_usd_brl`) resolves via `_spot_usd_brl_rate` (`calculate.py`): live `weighted_avg_rate` → `last_nonzero_avg_rate` → constant 5.0, the constant warned on stderr (`[irr_calculator] spot FX: …`) — never silent. When prices are fetched, the `USD_BRL` market indicator overrides this seed; on `--skip-prices` runs or indicator-fetch failures the chain's result stands.
