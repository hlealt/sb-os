...
| D21 | Gate thresholds carregam provenance (cp-sb-bookkeeper-gates-measure-meaning, change 3) | Cada threshold em `standing-rules.yaml` consumido por `gate_coverage.py` (`gates.step_5_5_coverage.threshold` → metric `despesas_tagged_brl_pct`; `tag_coverage.gate.untagged_amount_threshold_brl` → metric `untagged_despesa_floor_brl`) DEVE carregar um bloco irmão `*_provenance` nomeando a metric para a qual o número foi decidido. `gate_coverage` recusa rodar (exit 2) se o bloco faltar ou nomear outra metric (threshold herdado) — impede a classe de bug de 2026-06-05 (um número decidido para uma metric vinculando silenciosamente outra). Helper puro: `lib/standing_rules.py::check_threshold_provenance`; conduct gêmea (agente): Rule C.1 em `workflows/sb-bookkeeper/gatekeeper-loop.md` |
