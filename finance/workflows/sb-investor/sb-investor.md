...
- The `sb-investor-loop.md` invariants and per-step Investor Checkpoint apply on EVERY turn — never skip the checkpoint, never silently execute an out-of-structure request (Rule A), never skip a required policy load. See `./sb-investor-loop.md` for the full invariant set.
