...
**Loaded by:** `./sb-investor.md` reads-and-follows this file when `./capability-manifest.md` routes the `portfolio` (B4) intent. Read `./sb-investor-loop.md` before acting on any step below.
...
Before ANY reasoning, load the policy file(s) `../../CLAUDE.md` § Policy Read-Rules requires — per `./sb-investor-loop.md` § Policy read-rules wiring. Mapping exposure and proposing any watchlist change are such actions: `research-policy.md` is required (scope / priorities / exclusions / watchlist-approval / horizon). NEVER restate the read-rules table — read it.

If `research-policy.md` marks an entity or position in scope's reasoning out-of-scope or excluded, say so for that item and STOP on it, or offer to widen scope via the `policy` thin mode — do NOT reason past an exclusion (`./sb-investor-loop.md` § Policy read-rules wiring; Rule A).

## Step 2 — Read exposure (tools-only)

Read position data ONLY through the registered `class: read` tools in `../../scripts/tools-index.md` — NEVER read `portfolio.json`, a ledger CSV, or a snapshot directly (`./sb-investor-loop.md` § Tools-only data access). Scan that index for `class: read` and select by the question each tool answers:
...
Treat each tool's stdout as the authoritative position record for this turn. If the position data the join needs has no read tool, that is out-of-structure → Rule A `[A]` (`./sb-investor-loop.md`): record the missing-read-tool gap and route it to the build; NEVER hand-read the store to compensate and NEVER build a tool at runtime. A tool reporting an anomaly (e.g. `position_summary` exit 1) or data the join cannot reconcile is surfaced per `./sb-investor-loop.md` § Issue-surfacing; a suspected data problem routes to `sb-bookkeeper` (never fixed here).
...
Match on the ledger id / ticker identity; when a `related_positions` entry does not resolve to any Step-2 position (or the reverse), classify it (stale mapping vs genuinely unmapped) and surface ambiguity per `./sb-investor-loop.md` § Issue-surfacing rather than guessing. A persistent pattern of missing or duplicated matches is the § Build-on-demand trigger.

## Step 5 — Present the coherence map (present-and-confirm)

Run `./sb-investor-loop.md` § Present-and-confirm. State the coherence map and STOP for the user's choice. The map MUST carry:
...
This map is read-only output: presenting it persists nothing. Any follow-up — authoring, reviewing, recording a decision, or a watchlist change — is the user's to choose. A request to **set `watchlist: true`** on any page that arises here is the watchlist invariant applied: surface it through `./sb-investor-loop.md` § Present-and-confirm and the `policy` thin mode; the agent NEVER auto-sets `watchlist` to clear a coherence gap or satisfy its own reasoning (`./sb-investor-loop.md` § Watchlist invariant).
