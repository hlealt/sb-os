...
The `sb-wiki-capture-source` tool, the web-search sub-agent, and the Step 7b extraction pair (scan sub-agent + `investment_financials_extract` write tool) are additional access mechanisms named in the `research` and `extract` capability rows. The **adversarial refuter** (below) is a further shared CROSS-mode mechanism — distinct from the three above and reachable ONLY from inside a refuter-enabled mode's checkpoint, never as a standalone intent.

**Wiki-page discovery** is a further access mechanism — the deterministic `sb-wiki-search.py` semantic+keyword search — that `review`, `research`, and `thesis` use to FIND relevant wiki pages search-first (additive to grep + direct reads). Its shared procedure, probe-gating, and fail-soft posture are defined in `./sb-investor-loop.md` § Wiki-page discovery (never restated here).
...
**Step 7a Disconfirm is the stable, dispatchable interface that `thesis` (B1) and `review` (B3) reach by DISPATCHING `./disconfirm-wave.md` directly** — they NEVER re-implement discovery (single evidence engine). This rides the existing `review`→`research` sub-agent dispatch precedent. Consumers dispatch against this contract (authoritative in `./disconfirm-wave.md` — keep stable):
...
| Access mechanism | Read-and-follow `./thesis.md` (the reasoning) **+** invoke `sb-fin-create-thesis` in its investor-orchestrated mode (the only writer of the thesis page) **+** wiki-page discovery (`./sb-investor-loop.md` § Wiki-page discovery) |
...
| Field | Value |
|-------|-------|
| Fires on (intent) | "extract fundamentals from `<filing/source>`", "backfill `<entity>`'s Financials", "re-derive `<entity>`'s `## Financials` table", "verify the llm rows on `<entity>`" (the lint Fundamentals #2 spot-check action) |
| Access mechanism | Read-and-follow `./research.md` Step 7b steps 1–5 against ALREADY-captured raw **+** scan sub-agent dispatch (Sonnet; Opus opt-in for active-thesis sources) **+** call the registered tools `investment_financials_extract` (the SOLE `## Financials` writer) and `sb-wiki-capture-source` (lane-1 companyfacts companion capture only) per `../../scripts/tools-index.md` |
| Inputs | captured raw path(s) + the target entity page (must already exist — the tool never creates entities); the entity kind's vocabulary (`metric-vocab.md` incl. Suffix Families); `source-policy.md` § Capture User-Agent when the companion capture fires; any active-thesis focus |
| When to use | Fundamentals must flow from already-captured raw into an entity's `## Financials` with verified provenance — backfill, reconciliation, or verifying an `llm` row lint flagged. Authorization = the user's ask, gated ONCE: run `--dry-run`, present the post-extraction report, apply on confirm (`./research.md` Step 7b closing rule) |
| When NOT | A capturing research run extracts via Step 7b under its Ingest-gate `extract?` column — never route here mid-research. Raw not yet captured → `research`. Neither this route nor any agent hand-writes a `## Financials` row (`section-menus.ext.md` write path) |
