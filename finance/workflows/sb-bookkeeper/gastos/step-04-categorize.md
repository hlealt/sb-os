...
**Goal:** Run `categorize.py` to auto-classify transactions, then read its structured stdout to surface unknowns by item type (categories, suppliers, tags) — priming Pass 1 of the review queue in step-05.
...
3. Report the counts to the user in a single line:
   ...
   ```
   Unknown categories: {N1} · Unknown suppliers: {N2} · Unknown tags: {N3}
   ```
...
- **Gatekeeper checkpoint** → before advancing, run § Per-Step Checkpoint in `../gatekeeper-loop.md` (out-of-structure → Rule A; detected issue → Rule C blocking/deferrable; direct data read → re-route through a `tools-index.md` tool). The unknowns surfaced here (categories/suppliers/tags) are the deviation-to-structure protocol's input — Step 05 resolves them per Rule B.
