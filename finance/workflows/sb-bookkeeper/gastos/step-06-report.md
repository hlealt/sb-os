...
The monthly closing report (`{MONTH}-fechamento-mensal.md`) is no longer generated. The financial dashboard (rendered by install.py to `finance_dashboard_html_path` in `sb-os.json`, default `.user/finance/dashboard.html`) is the canonical view of monthly closing data.
...
- **[C] Continue** → proceed to Step 07 (Recurring Payments)
