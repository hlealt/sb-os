...
2. **Completion gate — tag coverage before commit (`gate_coverage.py`, gates #1/#3 — auto-LOOP).** Run over the final CSV:
   ...
   - **Exit 1 (FAIL) with `{LOOP_COUNT} < 3`** → this gate **auto-loops** (it is not an inline halt). Return to the tag batch (Section 1 § Batch — tags) and tag the expenses lacking coverage, or acknowledge legitimately untaggable large despesas by appending ack rows with the `ack_tag_review` tool (`python "{SCRIPTS_DIR}/ack_tag_review.py" --month {MONTH} --source review-{MONTH} --date <D> --description <TEXT> --amount <AMT> --apply` — the sanctioned writer of `tag-review-acks.csv`; dry-run is the default, so add `--apply`; NEVER hand-edit the CSV); increment `{LOOP_COUNT}` by 1, rewrite the CSV, and run the gate again. This repeats until exit 0 or until `{LOOP_COUNT}` reaches 3.
