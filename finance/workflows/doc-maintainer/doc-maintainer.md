...
**Skill front door (binding).** The `sb-doc-maintainer` skill is the user-invocable entry point: the Skill tool loads this workflow into the CURRENT session and the invoking agent BECOMES the caller-broker. It assembles the change spec below by eliciting `change` + `surface` from the user (the refusal on a missing change spec stands — no blanket rewrites), brokers the same reconcile/return iteration with the user directly, and owns every caller-side duty (surfacing doc diffs, staging the edits with the structural change). This is an entry-point addition, NOT a runtime-model change — every binding constraint (§ Authority Boundary, change-spec requirement, surgical scope) applies identically, and sibling agents KEEP dispatching this companion via the Agent tool per Seam 2. Reconciled 2026-06-06 (`../tool-builder/` carries the same front-door clause).
...
DATA_FLOW_MAP        = <UNSET — no default ships; every install must supply this via `.user/context/doc-maintainer/doc-maintainer.yaml`>
...
2. **`{DATA_FLOW_MAP}` resolution gate.** If the change spec touches the data-flow map and `{DATA_FLOW_MAP}` is unresolved (no path was injected via `.user/context/doc-maintainer/doc-maintainer.yaml`), STOP and tell the user to create that file with a `data_flow_map` key pointing at their install's data-flow-map path. Do not proceed and do not silently skip the data-flow-map reconcile.
3. **Authority-boundary pre-check.** Re-read § Authority Boundary. If `change` (or any `correction`) asks this companion to edit config DATA, ledgers, `portfolio.json`, or tool code directly, STOP and return the refusal verbatim to the caller. Update nothing.
4. **Resolve node → doc-section mapping.** Read `{DOC_CURRENCY_MANIFEST}` (the static node-doc lookup) to resolve each touched `node_id` to its data-flow-map node and the doc file(s)/sections that describe it. This bounds the reconcile to exactly the sections that describe the changed structure — never a whole-file rewrite.
...
| Failure | Behavior |
|---------|----------|
| Dispatch prompt asks for a direct config-data / ledger / `portfolio.json` / tool-code write | REFUSE (Step 1.3). Update nothing. Return the boundary refusal verbatim to the caller, naming the correct owner. |
...
| Change spec touches the data-flow map and `{DATA_FLOW_MAP}` is unresolved (Step 1.2) | STOP. Tell the user to create `.user/context/doc-maintainer/doc-maintainer.yaml` with a `data_flow_map` key. Do not proceed and do not silently skip the reconcile. |
