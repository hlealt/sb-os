...
```yaml
tool: investment_source_capture (python investimentos/investment_source_capture.py --url URL --origin ORIGIN [--mode markdown|html-archive|both|browser|manual] [--title TITLE] [--thesis SLUG] [--vault-root PATH] [--dry-run] [--gated] [--gated-why TEXT])
purpose: Save an approved open-web URL to {wiki_root}/raw/{origin}/ and return a metadata summary only (title, url, origin, related thesis, saved path, lifecycle state, byte count); gated sources register gated_pending_access in log.md without fetching.
owner_script: investimentos/investment_source_capture.py
class: write
use: parser
expected_inputs: --url (required); --origin folder name (required); --mode (markdown|html-archive|both|browser|manual, default markdown); optional --title, --thesis slug, --vault-root, --dry-run, --gated, --gated-why; reads {wiki_root}/raw/{origin}/ (wiki raw store)
outputs: JSON metadata summary to stdout (state, url, title, origin, related_thesis, saved_paths, bytes, dry_run); for gated: state=gated_pending_access + log_path; for blocked: state=blocked + error. Writes raw .md or .html file(s) to {wiki_root}/raw/{origin}/. Dry-run writes nothing.
