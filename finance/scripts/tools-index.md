...
```yaml
tool: investment_source_capture (python investimentos/investment_source_capture.py --url URL --origin ORIGIN [--mode markdown|html-archive|both|browser|manual] [--ext md|html|json] [--title TITLE] [--thesis SLUG] [--vault-root PATH] [--user-agent UA] [--manual-file PATH] [--no-curl-fallback] [--dry-run] [--gated] [--gated-why TEXT])
