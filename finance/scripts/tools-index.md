...
outputs: JSON metadata summary to stdout (state, url, title, origin, related_thesis, saved_paths, bytes, fetch_method, dry_run; manual path adds manual_source); for gated: state=gated_pending_access + queue_path + queue (registered | already-registered | dry-run); for transport blocked: state=blocked + error (both-fail error names both methods) + queue_path + queue. Writes raw .md or .html file(s) to {wiki_root}/raw/{origin}/ (manual path always .md). Creates {wiki_root}/source-queue.md with type: source-queue frontmatter when absent. Dry-run writes nothing.
canonical_reader_writer: writes {wiki_root}/raw/{origin}/<YYYY-MM-DD-slug>.{md,html}; appends {wiki_root}/source-queue.md (gated + transport-blocked registrations)
dry_run: available
