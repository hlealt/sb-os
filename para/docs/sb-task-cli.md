...
- **Refs: two identities, only one durable.** A `<ref>` resolves in this order — task NUMBER (exact) · LINE ref `L<n>` (any line inside the block, from the `line` column of `list`/`deps`) · title (exact, then unique substring). The two namespaces cannot collide: `check_number` admits digits/dots + optional letter only, so no number is ever `L`-shaped. A line ref is a TRANSIENT convenience for the task at hand — it is invalidated by any edit above it and MUST NEVER be persisted (`_Depends:_` / `_Done-after:_` accept numbers only, deliberately).
