...
description: Interactive CRUD for skill, workflow-step, and command context injection entries
---

# Context Injection Manager

Manage user-specific context entries for skills, workflow steps, and slash-commands. Creates, updates, and deletes YAML context files under the configured user-context root (resolved from `sb-os.json` → `user_context_root`; default `.user/context/`).
...
python {sb_os_path}/para/workflows/sb-inject-context/resolve_context.py --surface command --name <command-name>   --path-only
...
```
python {sb_os_path}/para/workflows/sb-inject-context/resolve_context.py --surface <skill|command|step> <--name <skill-or-command-name> | --file <step-file-path>> --path-only
```
...
- If the YAML's parent directory does not exist → create it (`{user_context_root}/skills/` for a skill, `{user_context_root}/commands/` for a command, `{user_context_root}/{workflow-path}/` for a workflow step)
...
Same as Create step 1 — determine the surface (skill, command, or workflow step) and resolve to its YAML path.
...
- When creating new files, include a comment header: `# Context for: skill [skill-name]` (skill target), `# Context for: command [command-name]` (command target), or `# Context for: [workflow] / [step]` (workflow-step target)
