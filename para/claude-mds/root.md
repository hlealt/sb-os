...
**Parallel sessions — commit collisions.** Staging a file commits ALL its uncommitted hunks, including other sessions'. Before committing a vault file, diff it against your own session's delta — either confirm the foreign hunks ride along (disclose them in the commit message) or stop and re-scope. **Then scope the COMMIT, not the staging — `git commit -- <path> [<path>…]` is the only bound that holds on a shared index**, because it re-resolves the paths at commit time. `git add` + a `git diff --cached` check + `git commit` does NOT: a parallel session staging in the window between the check and the commit rides along, and the check passed seconds earlier (measured 2026-08-11 — a commit intended to carry one file carried four, with both sessions following the stage-then-verify guidance correctly). To publish off a shared branch that has diverged, cherry-pick your commits into a temporary worktree OUTSIDE the vault and push from there — the shared working tree is never touched. NEVER `git commit --amend` in the vault: HEAD may have moved to another session's commit between your read and the rewrite — fix history forward with a new commit, or not at all.
...
**Parallel sessions — whole-tree operations.** On a shared working tree, `git stash`, `git restore`/`git checkout` with no explicit pathspec, `git checkout -- .`, and `git clean` are FORBIDDEN — each silently reverts or discards every concurrent session's uncommitted work at once, defeating the write-collision and revert protections above in one shot, and is invisible to the sessions it hits (measured 2026-08-31, redesign-continue-1: FIVE seats of ~20 ran a whole-tree `git stash` A/B while peers held uncommitted edits — the tree was verified after every one and nothing was lost, so this is a measured near-miss series, not a data-loss incident; all five had read the prohibition by name, which is why the substitute below is stated here inline rather than elsewhere). The recurring real need behind the forbidden command is comparing your change against clean HEAD — do that instead with a scratch worktree outside the vault, never inside the tree you're comparing against:
