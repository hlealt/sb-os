...
- **`.user/`** is the user-owned root — holds the user-context folder (read by sb-os at the path declared in `sb-os.json` → `user_context_root`, default `.user/context/`) plus any personal extensions. sb-os creates the `.user/` directory on the initial fresh install (so the configured user-context path resolves), but never writes inside `.user/` thereafter — the configured user-context path is the only sb-os-readable subpath.
