...
| `../_shared/wiki/folder-structure.md` | 2, 3, 7 |
| `../_shared/wiki/index-formats.md` | 2 |
| `../_shared/wiki/naming-convention.md` | 4, 5, 7 |
| `../_shared/wiki/citation-format.md` | 5 |
| `../_shared/wiki/frontmatter-schemas.md` | 7 |
| `../_shared/wiki/section-menus.md` | 7 |
| `../_shared/wiki/log-entry-shapes.md` | 7 |
...
2. Classify candidate page types per `../_shared/wiki/page-types.md` Discriminator Rule:
...
3. Extract keywords — the substantive nouns and noun phrases in the question. Apply lowercase-kebab transformation per `../_shared/wiki/naming-convention.md` Slug Rules to derive candidate slug fragments. Build `keywords` set.
...
For each leaf folder selected by `candidate-types` from step 1 — `{wiki_root}/wiki/concepts/`, `{wiki_root}/wiki/entities/`, `{wiki_root}/wiki/topics/` per `../_shared/wiki/folder-structure.md`:

1. Read the leaf index file (`concepts.md`, `entities.md`, `topics.md`).
2. Apply expected formats per `../_shared/wiki/index-formats.md` Wiki Sources Index AND the leaf-index format conventions:
...
If `wiki/sources/` may also hold relevant material (the question references a specific source slug, an origin, or a date), add matching source pages from `{wiki_root}/wiki/sources/{*}/` to `picks` using the same scoring against the wiki sources index per `../_shared/wiki/index-formats.md`.
...
3. Determine if depth is needed: if the answer requires context from a wikilink referenced inside the picked page (e.g., a page's `Related` section names another page central to the question), follow the wikilink.
   - Resolve the wikilink to its target file per `../_shared/wiki/naming-convention.md` Wikilinks rules (target filename match is exact, including date format).
...
2. Cite every claim with inline `[[<page>.md]]` wikilinks for wiki pages AND inline `[^N]` markers for source pages per `../_shared/wiki/citation-format.md`.
...
4. Use wikilink format per `../_shared/wiki/naming-convention.md` Wikilinks rules — `[[<filename>.md]]` matching the target file's actual filename.
...
1. Derive a proposed topic slug from the question and the synthesized answer (lowercase-kebab per `../_shared/wiki/naming-convention.md`).
...
1. Derive the slug from the question and the synthesized answer (lowercase-kebab per `../_shared/wiki/naming-convention.md`). Verify it does not already exist at `{wiki_root}/wiki/{type}s/{slug}.md` — if it does, halt and surface the conflict; the user may rename or merge manually.
2. Verify slug collision rules per `../_shared/wiki/naming-convention.md` Type Folder Collision Rules (concepts vs entities allowed; concepts/entities vs topics forbidden).
3. Write `{wiki_root}/wiki/concepts/{slug}.md` (concept) or `{wiki_root}/wiki/entities/{slug}.md` (entity). Create the leaf folder lazily if absent per `../_shared/wiki/folder-structure.md`.
4. Frontmatter per `../_shared/wiki/frontmatter-schemas.md`:
...
5. Section structure per `../_shared/wiki/section-menus.md`:
...
6. Citations per `../_shared/wiki/citation-format.md`: copy the inline `[^N]` markers and matching `[^N]: [[<source-filename>.md]]` definitions from the step 5 answer into the new page's body and `Sources` section.

#### 7c — Append `query` log entry

Always append a `query` entry to `{wiki_root}/log.md` per `../_shared/wiki/log-entry-shapes.md`. Entry is an H2 heading: `## [YYYY-MM-DD HH:MM] query | <brief>`.
...
Per `../_shared/wiki/log-entry-shapes.md`, a `query` entry is standalone (NOT a sibling of any ingest entry). Use the current timestamp.
