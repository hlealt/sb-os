...
|-----------|-----------------|
| Workflow entry point | `{sb_os_path}/workflows/{id}/{id}.md` |
| Workflow step files | `{sb_os_path}/workflows/{id}/step-{nn}-{name}.md` |
| Workflow data files | `{sb_os_path}/workflows/{id}/data/` |
| Skills (auto-discoverable) | `{sb_os_path}/skills/{id}/SKILL.md` |
| Commands (manually invoked) | `{sb_os_path}/commands/{id}.md` |
| Rules | `{sb_os_path}/rules/{id}.md` |
| Templates | `{sb_os_path}/templates/` |
| Reference docs | `{sb_os_path}/docs/` |
...
Docs files document **final state only** — never logs or planning artifacts. Workflow directories must NOT contain documentation — all docs live in `{sb_os_path}/docs/`.
...
nextStep: {sb_os_path}/workflows/{id}/step-01-{first-step}.md
---
```
...
```markdown
Read and execute `{sb_os_path}/workflows/{target}.md`.
```
