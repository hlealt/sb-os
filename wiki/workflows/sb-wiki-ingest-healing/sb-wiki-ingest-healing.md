...
| one bare token naming an origin folder | That origin's heal-index `heal=yes` rows — the un-healed queue for that origin. Already-healed pages (`heal=no`, carrying a `healed:` stamp) are SKIPPED. Resolved like the sweep, SCOPED to the origin (NOT the manifest's `origin` mode, which would re-target every folder page). To deliberately re-heal a healed page, name it explicitly (next row) or re-mark it `heal=yes` in the dashboard. |
| one-or-more `origin/stem[.md]` source-page refs, or ≥2 tokens | Exactly those pages (`--healing` → `files` mode) — an explicitly-named page heals EVEN IF already healed (force re-heal). A bare origin token among them expands to that origin's `heal=yes` rows (skipping healed), never to all its folder pages. A ref resolving to no page lands in `skipped_not_ingested` and NEVER halts the run. |
...
| Heal-index is the queue | Multi-target / `heal all` / bare-origin selection reads the heal-index `3-resources/knowledge-base/heal-index.md` and takes the `heal=yes` rows (each row's `wiki` path = a source page to heal; a bare origin token takes only that origin's `heal=yes` rows, so already-healed pages are skipped). An EXPLICITLY-named `{origin}/{stem}.md` page is the sole exception — it force-heals even when already healed. At close-out a healed source's row is flipped to `heal=no` (whole-table rewrite — never a single-cell edit) so the source leaves the queue. The scan/check mode (Step 0) MERGE-refreshes the heal-index: new sources appended as `heal=no`, existing `heal` values never overwritten. |
...
- **A bare origin token (e.g. `lennys-podcast`) — the per-origin queue:** do NOT forward the bare origin to the manifest as `--origin` (that resolves ALL the origin's folder pages, re-healing already-healed ones). Instead resolve it like the sweep, SCOPED to that origin: read the heal-index and collect every `heal=yes` row whose `wiki` path is under `wiki/sources/{origin}/`; pass each as a positional `{origin}/{stem}.md` target. Already-healed pages (now `heal=no`) are thereby SKIPPED. A `large`/`small` size keyword still composes — pass it through and the manifest filters the collected targets to that bucket. If the origin has zero `heal=yes` rows, STOP: "no `heal=yes` targets in `{origin}` — already fully healed". To deliberately re-heal a healed page, name it explicitly (next bullet) or re-mark it `heal=yes` in the dashboard.
- **Explicit `{origin}/{stem}[.md]` page refs:** forward them VERBATIM as positional targets — an explicitly-named page heals EVEN IF already healed (force re-heal; the durable `healed:` stamp does not block an explicit single-page target). A bare origin token mixed in among page refs expands to that origin's `heal=yes` rows (as above), never to all its folder pages. Record which sources were healed — Step 5 flips each row to `heal=no`.
