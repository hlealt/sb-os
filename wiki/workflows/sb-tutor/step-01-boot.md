...
2. If no `pref.learning.profile` entry is present (first profiling run — independent of whether other entries exist):
   ...
   b. Run a brief learning-style diagnostic inline (no external file). Acting as an educational psychologist, present 10 scenario-based questions — in batches of 3 — that probe how the student prefers to take in and work with new material, to identify their primary and secondary learning modalities (visual / auditory / reading-writing / kinesthetic) and their learning-environment needs. Keep it brief — gather the answers, then synthesize the profile.
