...
| `../shared/frontmatter-schemas.md` | 2 |
| `../shared/section-menus.md` | 2 |
| `../shared/citation-format.md` | 2 |
| `../shared/naming-convention.md` | 2 |
...
Write `{wiki_root}/wiki/topics/{slug}.md`. Create the `wiki/topics/` folder if it does not exist (lazy creation per `../shared/folder-structure.md`).

Frontmatter per `../shared/frontmatter-schemas.md` Topic schema (common only — no Topic-specific additions):
...
Section structure per `../shared/section-menus.md` Topic Page entry. Required sections: `Scope`, `Sources`. Optional sections: pick per the topic shape determined in step 1 (debate / comparison / landscape / decision-frame / evolution) using the "When to include" mapping in the shared file.
...
2. Select optional sections per topic shape from `../shared/section-menus.md` Topic Page entry — agent picks per shape AND per source signal. Do NOT include all optional sections by default.
3. Cite every claim with inline `[^N]` markers per `../shared/citation-format.md`. Append matching `[^N]: [[<source-filename>.md]]` definitions in the `Sources` section.
...
2. Locate or create a `Related` section per `../shared/section-menus.md` (Concept and Entity optional menus both include `Related`).
