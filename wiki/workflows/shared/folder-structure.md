...
- Per-kind subfolders WITHIN a type folder are an opt-in subdivision pattern proposed by lint (`/sb-wiki-lint`) when one kind crosses **≥10 pages** (SOLE AUTHORITY FOR THIS VALUE — all other files defer here). Pre-subdivision, every type folder is flat. Schema § "Folder subdivision" defines naming policy (e.g., `kind: model` → `ai-models/` because "models" is generic), parent-index router format, and the managed `{type}/CLAUDE.md` marker block. Subdivision is opt-in per-kind; mixed structure (some kinds in subfolders, others flat) is permitted.
...
- Raw files are IMMUTABLE in CONTENT — never edit a saved source file. Two permitted exceptions, both owned by `/sb-wiki-ingest` and `/sb-wiki-lint`: (1) a title-conformance RENAME of a PDF (filename only, content untouched) per `../shared/naming-convention.md` § "Raw PDF Title-Conformance"; (2) **Twin regeneration (OD-3)** — a PDF's DERIVED text twin (`{title-slug}.md`) MAY be REGENERATED from the still-immutable PDF by a better extractor (overwritten in place under the same filename). The PDF itself is the untouchable original; the twin is a regenerable derivative, NOT a saved source. This exception covers ONLY a PDF's text twin — every other raw file (markdown clips, transcripts, the PDFs themselves) stays content-immutable.
