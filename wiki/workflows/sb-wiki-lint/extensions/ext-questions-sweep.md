...
> **Validation window — CLOSED (owner-ratified).** The graduation maturity heuristic is frozen: an answered `questions.md` entry surfaces as mature once it has **≥2 accreted `answer:` bullets from distinct sources, OR a single bullet the user has marked**. The window closed after the heuristic was exercised at real scale — its first GRADUATION PROPOSAL surfaced and the owner confirmed the maturity judgment picked correctly. The **duplicate-of-existing-topic guard** below (separate from this frozen maturity wording) gates graduation on whether the question is already resolved inside an existing topic, so it never proposes a page that duplicates content already folded into a broader topic. Per `../../../docs/wiki-schema.md` § "Questions layer — questions.md" → "The answer-scan" validation-window note (heuristic 1, graduation maturity).

**Duplicate-of-existing-topic guard (applied AFTER maturity, BEFORE adding to `graduation-proposals`).** A mature entry is NOT proposed for graduation if an existing TOPIC already resolves it — graduating would duplicate content already folded into a broader topic. Tier-gated, reusing the wiki's trusted semantic signal:
- **Semantic tier available** (§ "Retrieval tiers — hybrid search"): probe the question against topics — `search "<question text>" --type topic --k 5 --no-rerank` (one call per mature entry). For each hit, READ its `Scope` + relevant headings (bounded partial read) and judge, at the SAME bar as `sb-wiki-create-topic`'s `extend N` overlap check, whether it already answers the question. Covered → SUPPRESS the graduation proposal and route the entry through 7.7a's answer-sweep as resolved-by `[[<topic>.md]]` (its answer folds into that topic and the entry retires) instead of spawning a duplicate page. Not covered → proceed to `graduation-proposals`.
- **Semantic tier unavailable**: skip the guard (propose as before); `sb-wiki-create-topic`'s own `extend N` overlap check is the accept-time backstop (it caught exactly this duplicate on 2026-06-13).
