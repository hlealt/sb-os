...
| Firm topic updates (`TOPIC UPDATES (firm …)` block — genuine firm-tier entries ONLY, answer-origin entries excluded) | **`accept` ALL — apply each via the staged Step 4.5 update** (Step 4.5 owns the apply-semantics — sole authority). Write ONE audit record per applied update into the summary `Flags` field (see Audit records below). This is the v5 silent-mode change — interactive mode now applies firm on commit. |
