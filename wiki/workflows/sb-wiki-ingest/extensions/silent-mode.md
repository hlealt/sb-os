...
**Regenerable-twin exemption applies here too** (Step 1.7 body, "check FIRST"): a raw that is itself a regenerable PDF twin whose canonical `.pdf` is already ingested is ALREADY-INGESTED — RETURN `committed` with nothing to do (or the caller's already-ingested skip), NEVER `failed (content-duplicate)` and NEVER a `Duplicate`/`Original` row. Marker-based recognition — a diverged twin stem (title-conformance PDF rename) must never read as a duplicate.
...
| Firm topic updates (`TOPIC UPDATES (firm …)` block — genuine firm-tier entries ONLY, answer-origin entries excluded) | **`accept` ALL — apply each via the staged Step 4.5 update** (Step 4.5 owns the apply-semantics — sole authority), **EXCEPT any target the injected capture-only guard below protects** — hard-skip the body write and route per the guard. Write ONE audit record per applied update into the summary `Flags` field (see Audit records below). This is the v5 silent-mode change — interactive mode now applies firm on commit. |
| Semantic topic updates (SEMANTIC TOPIC UPDATES) | **QUEUE all** — append each to `{wiki_root}/pending-topic-updates.md` (dedupe-keyed by (source, topic) pair; create the file with a minimal header if absent); one `Flags` line per queued row; NEVER apply unattended, NEVER drop. |
...
**Injected capture-only guard (MANDATORY — firm auto-apply is SUBORDINATE to it).** Before auto-applying ANY firm topic update, check the target topic against every injected-context directive that marks a topic (or a topic family, e.g. by slug glob) **capture-only / no-auto-write**. When the target matches such a directive, you MUST hard-skip the topic-body write and instead route the update to the capture destination that directive names — NEVER write the protected topic's body in silent/bulk mode, regardless of firm-tier confidence. Record it as a `Flags` line naming the topic, the capture destination, and the citing source. This guard exists because a firm-tier fire is mechanical (related-links driven) and will otherwise write a protected topic body before the worker weighs a directive buried in loaded context — the firm-apply step itself is the enforcement point, not the worker's memory of the directive. When no injected directive marks the target, apply firm normally per the table above.
...
| Semantic update queued | `semantic-update queued: [[<topic>.md]] ← [[<raw-filename>]] (semantic: <score>)` |
