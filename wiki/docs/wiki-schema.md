...
**Tie-breaker**: when in doubt, start as Concept or Entity. Promote to Topic only when the **durability bar** clears (§ "Topic creation rules" → "Granularity gate"): ≥3 distinct sources have engaged the tension, OR it answers a `questions.md` entry, OR the owner asks. A bare two-source clash with a single home page stays a Disputed callout / home citation — not a topic.
...
A raw **PDF** filename MUST equal the kebab-slug of the paper's actual title (the title printed on the document; for an ingested PDF, mirrored in the source page's `title:` frontmatter — the raw index no longer carries a `Title` column, ADX-9/ADX-10). Cryptic publisher/repository names (arXiv IDs like `2602.21012v1.pdf`, scan dumps like `kolmbook-eng-scan.pdf`) do NOT reflect the title and are renamed. Scope: PDF raw sources ONLY — markdown raw sources arrive title-named from the clipper and are exempt.
...
| Title source | The title on the document; for an ingested PDF the maintained record lint compares against is the source page's `title:` frontmatter (else first H1) — the raw index no longer carries a `Title` column (ADX-9/ADX-10). A not-yet-migrated legacy 4-col raw index still supplies it in transition. |
...
| Rule | Statement |
|------|-----------|
| Canonical header | `\| File \| Wiki \|` (ADX-9/ADX-10 — the summary column `Title`/`Description` AND the `Date` column are both DROPPED; neither had any programmatic reader). Recognized legacy headers: `\| File \| Title \| Date \| Wiki \|` and `\| File \| Description \| Wiki \|` (still read, and MIGRATED to `\| File \| Wiki \|` by `/sb-wiki-lint`; each row's `Wiki` value is preserved verbatim, the summary + date cells dropped). |
| `Wiki` is the row's final cell | The `Wiki` flag is the LAST cell of every recognized raw row (2-col canonical, 3-col legacy, AND 4-col legacy). A writer locates `Wiki` by the MATCHED ROW's own width (`len(row) - 1`), NEVER by the header's `index("Wiki")` — so a legacy 4-col data row appended under a 3-col header is flipped at its own last cell, never the wider row's Date cell; a 2-col row flips at index 1. |
| Width-sized appends | An appended row is sized to the index's ACTUAL header. The canonical 2-col header receives a 2-col `\| [[file]] \| No \|` row; a not-yet-migrated legacy header receives a row of its own width until the migration collapses it. The writer never manufactures a wider row under a narrower header. |
| Unrecognized width refused | A row whose width is none of 2/3/4 is REPORTED, never auto-flipped or guessed. |
| **D1 — canonical `.pdf` row** | For a PDF source the raw row keys on the **`.pdf`** (the immutable original). The regenerable `.md` twin (the page rendered by `sb-wiki-pdf-twin.py`, carrying `twin_extractor:`/`source_pdf:` frontmatter, or a legacy `Original PDF:` reference) gets **NO separate row** and is EXCLUDED from the row-adding sweep. Twin recognition is **MARKER-based, NOT same-stem** (`resolve_twin_pdf`): a twin resolves to a `.pdf` in its folder via `source_pdf:`, same-stem, an `Original PDF:` ref, or a sole in-folder PDF — so a title-conformance PDF rename that diverges the twin's stem from the PDF's never un-recognizes the twin (the Trinity misfire, 2026-07-02). Complementary invariant: the title-conformance rename executor (`--execute-renames`) renames a same-stem twin `.md` in lockstep with its PDF and refreshes its `source_pdf:`, keeping the pair aligned. A coexisting `.pdf`+`.md` twin pair is collapsed to the `.pdf` row by lint reconciliation. |
| D1 exception — dated clips | A dated CLIP `.md` (no `twin_extractor:` / `source_pdf:` / `Original PDF:` marker, e.g. caiso / engie-brasil daily clips) is NOT a twin and keeps its own row. |
...
The raw index (`raw/{origin}/{origin}.md`) is `| File | Wiki |` (ADX-9/ADX-10 — the summary `Title`/`Description` and `Date` columns were dropped; neither had a programmatic reader, and the publish date lives in source-page frontmatter post-ingest). The `Wiki` cell is factual only, no opinion. Raw indexes are created and maintained by lint (see `/sb-wiki-lint`), which also migrates legacy 4-col/3-col raw indexes to the 2-col form, preserving each row's `Wiki` value verbatim.
...
A fired trigger logs a `candidate-topic` ONLY when the tension clears the **durability bar**; below the bar it is captured at lower granularity and NO candidate is logged. This biases the wiki toward GENERAL, durable topics over narrow one-off clashes a single existing page already holds. The two escape hatches (question-anchored, owner-requested) let a genuinely-wanted narrow topic through.

**Durability bar — log a `candidate-topic` when ANY ONE holds:**

| # | Condition | How to count |
|---|-----------|--------------|
| 1 | **Accretion ≥3 sources** — ≥3 distinct sources have engaged the tension, not a 2-source one-off | Count distinct SOURCES engaging the tension, NOT the number of sides — a binary debate carried by many sources clears; a 2-source clash does not. Per trigger: Contradiction → sources cited on the affected page for the opposing claims + the new source(s); Evolution → distinct dated sources; Cross-application → distinct sources referencing the pairing. ≥3 → clears. |
| 2 | **Question-anchored** (escape hatch) | Answers a `questions.md` entry. |
| 3 | **Owner-requested** (escape hatch) | The owner explicitly asks for that specific topic. |

**Below the bar (unconditional capture, added regardless):** Contradiction → the `> [!warning] Disputed` callout on the affected page; Evolution → both dated sources cite the home page (no new marker); Cross-application → no candidate (re-detected on a later ingest). **Graduation is automatic at ingest** — when a later source pushes the distinct-count to ≥3 (bar 1) the trigger re-fires and proposes the topic THEN (batch-safe; no owner or separate lint pass needed). Cross-application is subject to bar 1 (≥3 sources), not an auto-pass — its two pages are definitional, not a durability signal. Operational spec: `../workflows/sb-wiki-ingest/data/candidate-topic-triggers.md` § "Granularity Gate".

### Existing topic updates (ingest)

Topic pages are plural-framed and accrete substance over time as new sources land in their scope. During ingest, the agent detects which existing topic pages this source plausibly extends, and proposes those updates at Stage 1 alongside PROPOSED TOPICS. The agent NEVER auto-appends to a topic page without explicit acceptance — and for GENUINE firm-tier rows (Step 3·7 mechanical detections; answer-origin entries excluded — they stay in `PROPOSED ANSWERS`, default-reject) the Stage-1 COMMIT IS that explicit acceptance: any committing response applies them minus rows the user explicitly `reject N`s, and `abort` rolls back everything (see Operations § "/sb-wiki-ingest" step 10). Semantic-tier and speculative-tier rows stay default-reject — an explicit `accept N` applies them; omission rejects.

**Relevance detection — three tiers.** A source is a candidate update for an existing topic page in the FIRM tier (mechanical wikilink/slug match — high confidence), the SPECULATIVE tier (new-stub conceptual fit — low confidence, capped, default-reject), or the SEMANTIC source-level tier (a citable claim extending the topic with no mechanical trigger — heuristic, capped, default-reject; see "Semantic tier (source-level)" below).
...
**Speculative tier (new-stub conceptual fit).** A (new stub, existing topic) pair fires when the stub qualifies AND the active firing signal holds. **Semantic-primary (D11):** when the semantic tier is available the membership signal is the SOLE decider — `token_overlap` does NOT fire on its own; it is the degrade path used ONLY when the tier is down. The mechanized token floor junk-fired at 31.6% on real sources where semantic membership fired zero false positives, so token-overlap can no longer surface a speculative fire while the trusted signal is available.

| Condition | Definition |
|-----------|------------|
| New stub | Candidate is a `stub-candidates` entry created in THIS ingest run — never an existing page |
| Semantic membership (PRIMARY — the decider when the tier is available) | When the semantic tier is available (§ "Retrieval tiers — hybrid search"), a (stub, topic) pair fires when the stub's probe call (one per stub-candidate, shared with the near-duplicate probe — see Stub policy § "Near-duplicate probe") returns that topic page among its results. While the tier is up this is the ONLY firing signal — token overlap is suppressed entirely |
| Token overlap (DEGRADE path — fires ONLY when the semantic tier is down) | When the semantic tier is unavailable, a (stub, topic) pair fires when the stub's preamble (1–2 sentence factual sentence written at step 5) shares ≥2 substantive tokens with the topic's `Scope` text. Tokenization: lowercase, strip stopwords (`the/a/an/of/for/in/on/and/or/to/is/are/with/by/that/this/it/as`), preserve kebab-case as a single token (`marginal-returns-to-intelligence` matches `marginal returns` if both tokens appear in scope). Scope text comes from the topics leaf index `Scope` cells (`wiki/topics/topics.md`) — ONE read covers every topic; a topic missing its index row is read directly (its `Scope` section). When the tier is UP this signal never fires on its own |
| Cap | Maximum 2 speculative fires per ingest. If >2 candidates qualify, rank by the active signal — semantic fires by helper score (descending) while the tier is up; token fires by overlap count (descending) on the degrade path. Keep top 2, drop the rest silently — re-detected on future ingests of related sources (no log entry) |
| Dedupe | If a topic already fires firm for this source, suppress its speculative fire (firm wins) — applies to whichever signal fired |

Speculative-tier fires are heuristic in confidence — semantic membership when the tier is available (the wiki's trusted signal per D11), token overlap only on the no-Voyage degrade (computed, not LLM-judged). The tier is rendered in a SEPARATE block at Stage 1 (`SPECULATIVE TOPIC UPDATES (low-confidence, default reject)`) and defaults to reject (unlike the firm tier, which applies on Stage-1 commit) — the separation signals confidence to the user. Each row's overlap cell names its signal: `semantic: <score>`, or `tokens: <t1>, <t2>` on the degrade path.

**Semantic tier (source-level).** A THIRD, additive tier that catches the gap the firm and speculative tiers miss: a source whose substance carries a citable factual claim extending an EXISTING topic, with NO mechanical firm trigger and no new stub to pair against. It is the wiki's trusted heuristic signal (D11: semantic membership fired zero false positives across 79 live queries where the mechanized token floor junk-fired at 31.6%), so it feeds a user-gated band — NEVER auto-apply.

| Aspect | Rule |
|--------|------|
| Detection (tier-gated; arm OFF when the tier is unavailable) | ONE probe per ingest, AFTER the run's first syncing call: `search "<source title> — <substance digest (≤2 sentences)>" --type topic --k 5 --no-sync`. No per-topic loop, no full topic walk. Tier unavailable (§ "Retrieval tiers — hybrid search") → arm OFF entirely; the run is today's mechanical baseline. NO token-overlap signal in any form — the no-Voyage degrade is "no widening", NEVER a token fallback (D11). |
...
**All three fires are gated** by the Granularity gate (§ "Topic creation rules" → "Granularity gate"): a `candidate-topic` is logged ONLY when the durability bar clears; the Contradiction `> [!warning] Disputed` callout and the Evolution home-citation are added regardless.
...
**Connectivity guard (live-structure promotion) — Step 3 clause 5b → 5c/6.** Operational spec: `../workflows/sb-wiki-ingest/extensions/purpose-lens.md` § "Step 3 clause 5b". Beyond the band bias in the table, the lens RESCUES a would-be-demoted discretionary stub (Title-only / Notable-Quote) that connects to live structure: if the name's HELD clause-5c near-duplicate probe hits include an existing TOPIC at SOLID semantic membership, it is created (sorted to `stub-candidates`) instead of demoted — reusing the probe, NO new helper call. An **orphan floor** then guarantees a source never lands with 0 wiki links (the single strongest stub fires anyway). This is a deliberate dial-UP on the discretionary stub surface keyed to live topic structure (an existing topic is a stronger, owner-curated relevance signal than `purpose.md`); mechanical → batch-safe (fires in `ingest-all`); creates only concept/entity stubs, never a topic page; tier-gated OFF when the semantic tier is down (orphan floor still fires via the base heuristic). Guarantees #2/#4 hold — only the discretionary surface is shaped, and never below baseline.
...
|------|-------|-----------------|
| **Topic `Open questions`** (stays on the topic page — menu UNCHANGED) | agent-authored gaps for *that* topic | the scan answers it → **strike the line, fold the answer into the topic body** via the existing firm `TOPIC UPDATES` append-only machinery. No entry ever moves into `questions.md`. |
...
| Where | Behavior |
|-------|----------|
| **Ingest** (`/sb-wiki-ingest`, active) | Load `questions.md` (skip if absent). Match the new source against open questions in both homes. **Semantic-primary (D11):** when the semantic tier is available, a candidate fires when the helper, queried with the open question text (`search "<question>" --k 5`), returns THIS ingest's source page among its results (the source page is on disk from step 2; the first helper call of the run self-syncs it into the index, later calls pass `--no-sync`); `token_overlap` is the DEGRADE floor — it fires only when the tier is down, NEVER on its own while semantic is available (the mechanized token floor junk-fired at 31.6% on real sources; semantic membership fired zero false positives). Then surface a **`PROPOSED ANSWERS`** block at the Stage-1 checkpoint alongside the existing proposal blocks. **Default reject.** On accept: topic-home → strike the line + fold the answer into the topic body (append-only + cite); `questions.md` → append an inline `answer:` bullet (cited). Silent mode auto-**rejects** all proposed answers (same posture as topic updates). |
...
> **Validation window.** (1) the **graduation maturity** heuristic is FROZEN (owner-ratified): an answered entry is "ripe" once it has ≥2 accreted `answer:` bullets from distinct sources, OR a single user-marked bullet — its validation window CLOSED after the first graduation proposal exercised the heuristic and the owner confirmed the maturity judgment was sound. A duplicate-of-existing-topic guard (lint 7.7b) gates graduation on whether the question is already resolved inside an existing topic, so it never proposes a page that duplicates content already folded into a broader topic. (2) the **scan firing signal** and (3) the **lint sweep thoroughness** are FROZEN (owner rulings D11/D12, validated 2026-06-10; the sweep-thoroughness ≥10-run validation window is owner-ratified CLOSED): the firing signal is semantic-primary — semantic membership (`search "<question>" --k 5` returns the source page) when Voyage is available, with `token_overlap ≥2` as the no-Voyage degrade only; re-running the ESL-2 inputs with live Voyage produced 0 junk answer-fires (was 33 token-fires) while a genuine pair ranked #1, and the lint sweep inherits the same frozen signal.
...
| Rerank stage | When a Voyage key is available, `search` sends the full RRF-fused candidate window to Voyage `rerank-2.5` by default and returns the top `--k` hits (default `--k` 25). A bounded type-boost (`RERANK_TYPE_BOOST`) re-weights each reranked candidate's relevance by page KIND — favoring concept/topic/thesis over verbose source pages — and re-sorts by the boosted key before truncating to `--k`; the reported score stays the raw rerank relevance (the boost changes ORDER only). The RRF-fill tail, dedup, range guard, exact-`--k` count, and never-empty-a-search invariants are preserved. `--no-rerank`, missing key, or rerank API failure → exact RRF order with the existing exit behavior (no type-boost). |
| Network timeout | Every Voyage HTTP call (embedding + rerank) carries an explicit per-request socket timeout (`VOYAGE_TIMEOUT`, 30 s) — a stalled or unreachable endpoint fails within the timeout rather than hanging the caller. On expiry the call degrades exactly like the no-key path: the query embed drops to FTS-only and a `search` self-sync answers from the current index, both with a stderr note and exit 0 (rerank degrades to RRF order, as above). |
...
**Concurrency safety — shared targets.** Cross-origin workers in a wave append to three GLOBAL files at once — `logs/mentions.md` + `logs/topics.md` (ingest Step 9) and `pending-topic-updates.md` (silent Step 10). Each append routes through the lock-guarded `wiki/scripts/sb-wiki-shared-append.py` (per-file lock + read-back verification), so no parallel append is lost. Shared EXISTING entity/concept/topic page edits (ingest Step 4 / 4.5) are NOT append-helper writes; the subagent dispatch prompt instructs workers to re-read-and-reapply their delta on a "file modified since read" rejection (general-purpose workers do not inherit the vault's recovery discipline). Same-origin batches stay serialized; the per-origin index writes remain protected by `sb-wiki-index-transaction.py`.
...
| 7.6 | **PDF title-conformance detection.** For each PDF in `raw/{origin}/`, compare the stem to the kebab-slug of the PDF's title — from the source page's `title:` frontmatter for an ingested PDF (the raw index no longer carries a `Title` column, ADX-9/ADX-10), or a not-yet-migrated legacy 4-col raw-index Title in transition (Naming convention § "Raw PDF title-conformance"). Mismatch + no name collision → `rename-proposals` row; mismatch + `{title-slug}.pdf` already exists → `duplicate-raws` finding (no rename). Detection only — execution is USER-GATED at step 9, updating the full referrer set per "PDF title-conformance (lint)" below. Markdown sources exempt. |
...
Step 7.6 compares each raw PDF stem to the kebab-slug of its title (the source page's `title:` for an ingested PDF — the raw index no longer carries a `Title` column, ADX-9/ADX-10; a legacy 4-col raw index supplies it in transition). Mismatches surface as a RENAME PROPOSAL block; execution is user-gated (same model as subdivision). On user `accept`, for each rename the agent updates the FULL referrer set atomically — a filesystem rename does NOT trigger Obsidian backlink updates:
