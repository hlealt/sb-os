...
| Section | Content |
|---------|---------|
| Pending rows | One row per (source page, topic) pair: `source page` \| `target topic` \| `signal` (`semantic: <score>`, or the firm match-type for a backfill-found firm gap) \| `proposed section` \| `proposed bullet + citation` \| `Decision` cell (blank = pending; owner marks `accept` / `reject`). |
