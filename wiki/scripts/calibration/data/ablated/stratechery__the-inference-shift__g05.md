<!-- SILVER-ABLATION FIXTURE -- DO NOT INGEST. grade=0.05 deleted=2 from=3-resources/knowledge-base/wiki/sources/stratechery/the-inference-shift.md schema=silver-ablation/1 -->
---
type: source
created: 2026-06-14
...
- AI compute is becoming heterogeneous rather than GPU-monolithic: while [[gpu.md]] hardware (led by [[nvidia.md]]) has carried both the [[ai-training-workload.md]] and [[ai-inference.md]], the future splits across distinct chip architectures matched to distinct workloads, of which Cerebras' May-2026 IPO (priced up to $150–160/share from $115–125 as demand surged) is one signal.[^1]
...
- Inference decomposes into prefill (parallel, compute-bound) and two interleaved decode steps (serial, memory-bandwidth-bound); for every token generated both the growing [[kv-cache.md]] and the full model weights must be read, making decode the latency-critical, memory-bound part of [[ai-inference.md]].[^1]
...
- Thompson projects [[agentic-inference.md]] becomes by far the largest market because it scales with compute rather than with humans or time, while [[answer-inference.md]] stays meaningful but relatively small and the [[ai-training-workload.md]] stays [[nvidia.md]]-dominated (high-speed compute, large HBM, high-speed networking).[^1]
...
- "Maybe the most profound implication of agents that act without humans in the loop ... will be that Moore's Law doesn't matter, and that the way we get more compute is by realizing that the compute we have is already good enough."[^1]
